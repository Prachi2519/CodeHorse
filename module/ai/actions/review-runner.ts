...
export const runPullRequestReview = async ({
  owner,
  repo,
  prNumber,
  userId,
  ...
  if (
    ...
    return {
      success: false,
      skipped: true,
      reason: "Review run requires owner, repo, prNumber, and userId",
    };
  }
  ...
  await upsertRunSnapshot({
    queuedReviewId:
      typeof queuedReviewId === "string" ? queuedReviewId : undefined,
    repositoryId,
  ...
  const context = await (async () => {
    ...
    try {
      ...
      console.warn("Skipping repository context retrieval:", error);
      return [];
    }
  })();

  const structuredReview = await (async () => {
    if (reviewMode === "merge_recap") {
